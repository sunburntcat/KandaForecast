...
async function get_ascensionwx( level=1020, all_flag=false, location='all')
{
    let locations;
    if (location === 'all')
    {
    ...
    for (const loc of locations)
    {
        ...
        for( const launch_fields of launches )
        {
            ...
            let observations = await rpc.get_table_rows({
                ...
                limit: 5000,
                lower_bound: launch.launch_id,
                upper_bound: launch.launch_id
            });
...
async function watch_ascensionwx( func, location='any')
{
    const contract = "ascension" + location;
    ...
    client.onConnect = () => {
        ...
        console.log("Listening to observations table on contract "+contract+" ...");
    }
...
exports.get_ascensionwx = get_ascensionwx;

exports.watch_ascensionwx = watch_ascensionwx;